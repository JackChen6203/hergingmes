...
# ── Decode auth.json from Base64 secrets (uploaded by deployer GUI) ──
# Hermes looks for OAuth credentials at ~/.hermes/auth.json
# Codex CLI looks at ~/.codex/auth.json
HERMES_DOT_DIR="${HOME}/.hermes"
CODEX_DOT_DIR="${HOME}/.codex"
mkdir -p "$HERMES_DOT_DIR" "$CODEX_DOT_DIR"

if [ -n "${HERMES_AUTH_JSON_B64:-}" ]; then
  echo "Decoding Hermes auth.json from HERMES_AUTH_JSON_B64..."
  echo "$HERMES_AUTH_JSON_B64" | base64 -d > "$HERMES_DOT_DIR/auth.json" 2>/dev/null && \
    chmod 600 "$HERMES_DOT_DIR/auth.json" && \
    echo "  ✓ ~/.hermes/auth.json restored ($(wc -c < "$HERMES_DOT_DIR/auth.json") bytes)" || \
    echo "  ✗ Failed to decode HERMES_AUTH_JSON_B64"
  # Also copy to $HERMES_HOME for backup
  cp -f "$HERMES_DOT_DIR/auth.json" "$HERMES_HOME/auth.json" 2>/dev/null && chmod 600 "$HERMES_HOME/auth.json" || true
fi

if [ -n "${CODEX_AUTH_JSON_B64:-}" ]; then
  echo "Decoding Codex auth.json from CODEX_AUTH_JSON_B64..."
  echo "$CODEX_AUTH_JSON_B64" | base64 -d > "$CODEX_DOT_DIR/auth.json" 2>/dev/null && \
    chmod 600 "$CODEX_DOT_DIR/auth.json" && \
    echo "  ✓ ~/.codex/auth.json restored ($(wc -c < "$CODEX_DOT_DIR/auth.json") bytes)" || \
    echo "  ✗ Failed to decode CODEX_AUTH_JSON_B64"
  # If Hermes auth.json not present, use Codex auth as fallback
  if [ ! -f "$HERMES_DOT_DIR/auth.json" ]; then
    cp "$CODEX_DOT_DIR/auth.json" "$HERMES_DOT_DIR/auth.json"
    chmod 600 "$HERMES_DOT_DIR/auth.json"
    echo "  ✓ Copied Codex auth.json → ~/.hermes/auth.json (fallback)"
  fi
fi
...
if "tokens" in data and "providers" not in data:
    ...
    normalized = {
        ...
        "active_provider": data.get("active_provider", "openai-codex"),
    }
    ...
    if isinstance(providers, dict):
        ...
        if isinstance(state, dict):
            ...
            if isinstance(tokens, dict):
                ...
                if isinstance(pool, dict) and not pool.get("openai-codex"):
                    pool["openai-codex"] = [
                        {
                            "provider": "openai-codex",
                            "source": "providers-openai-codex",
                            "auth_mode": state.get("auth_mode", "chatgpt"),
                            "access_token": tokens.get("access_token", ""),
                            "refresh_token": tokens.get("refresh_token", ""),
                            "account_id": tokens.get("account_id", ""),
                            "last_refresh": state.get("last_refresh") or data.get("last_refresh"),
                        }
                    ]
...
normalize_codex_auth "$HERMES_DOT_DIR/auth.json"
normalize_codex_auth "$CODEX_DOT_DIR/auth.json"
if [ -f "$HERMES_DOT_DIR/auth.json" ]; then
  cp -f "$HERMES_DOT_DIR/auth.json" "$HERMES_HOME/auth.json" 2>/dev/null && chmod 600 "$HERMES_HOME/auth.json" || true
  echo "  ✓ Synced normalized Hermes auth.json → ${HERMES_HOME}/auth.json"
fi

CLOUDFLARE_WORKERS_TOKEN="${CLOUDFLARE_WORKERS_TOKEN:-${CLOUDFLARE_API_TOKEN:-}}"
export CLOUDFLARE_WORKERS_TOKEN

# ── Resolve Telegram API host (DNS workaround for HF Spaces) ──
# Skip direct api.telegram.org host pinning when Cloudflare proxying is enabled:
# the direct Telegram IPs are often blocked from HF, while the Worker route works.
if [ -z "${CLOUDFLARE_WORKERS_TOKEN:-}" ] && [ -z "${CLOUDFLARE_PROXY_URL:-}" ]; then
  echo "Resolving Telegram API host..."
  sudo node "$APP_DIR/resolve-telegram-host.mjs" || true
else
  echo "Skipping direct Telegram host resolver; Cloudflare proxy is configured."
fi
...
PROVIDER_FOR_CONFIG="${HERMES_INFERENCE_PROVIDER:-${HERMES_PROVIDER:-auto}}"
...
case "$MODEL_PREFIX" in
  ...
  openai|openai-codex)
    ...
    [ "$PROVIDER_FOR_CONFIG" = "auto" ] && PROVIDER_FOR_CONFIG="openai-codex"
    MODEL_FOR_CONFIG="${MODEL_INPUT#*/}"
    ;;
  google|gemini)
  ...
  opencode-go)
    ...
    ;;
  # OpenAI models without prefix (gpt-5.5, gpt-5.4-mini, gpt-4.1, o3, o4-mini, etc.)
  gpt-*|o3|o3-*|o4-*)
    [ -n "$LLM_API_KEY" ] && export OPENAI_API_KEY="${OPENAI_API_KEY:-$LLM_API_KEY}"
    [ "$PROVIDER_FOR_CONFIG" = "auto" ] && PROVIDER_FOR_CONFIG="openai-codex"
    ;;
...
if [ "${TELEGRAM_MODE:-webhook}" = "polling" ] && [ -n "${TELEGRAM_BOT_TOKEN:-}" ]; then
  echo "Telegram polling mode enabled; clearing webhook..."
  python3 - <<'PY' || true
import os
import urllib.request

token = os.environ.get("TELEGRAM_BOT_TOKEN", "").strip()
proxy_url = os.environ.get("CLOUDFLARE_PROXY_URL", "").strip().rstrip("/")
base = proxy_url or "https://api.telegram.org"
if token:
    url = f"{base}/bot{token}/deleteWebhook?drop_pending_updates=false"
    try:
        with urllib.request.urlopen(url, timeout=20) as resp:
            print(f"Telegram deleteWebhook: {resp.read().decode('utf-8', 'replace')}")
    except Exception as exc:
        print(f"Telegram deleteWebhook skipped: {type(exc).__name__}: {exc}")
PY
fi
...
# ── Auxiliary models (vision, compression, title, search, web_extract) ──
# Use gpt-5.4-mini via openai-codex for lightweight tasks to save cost
aux_model  = os.environ.get("HERMES_AUX_MODEL", "gpt-5.4-mini").strip()
aux_provider = os.environ.get("HERMES_AUX_PROVIDER", "openai-codex").strip()
auxiliary = config.setdefault("auxiliary", {})
for task_name, task_defaults in [
    ("vision",          {"timeout": 120, "download_timeout": 30}),
    ("web_extract",     {"timeout": 180}),
    ("compression",     {"timeout": 120}),
    ("title_generation",{"timeout": 30}),
    ("session_search",  {"timeout": 30, "max_concurrency": 3}),
]:
    task = auxiliary.setdefault(task_name, {})
    task.setdefault("provider", aux_provider)
    task.setdefault("model", aux_model)
    task.setdefault("base_url", "")
    task.setdefault("api_key", "")
    task.setdefault("extra_body", {})
    for k, v in task_defaults.items():
        task.setdefault(k, v)
print(f"Auxiliary models set to: {aux_model} via {aux_provider}")

# ── Delegation (sub-agent) config ──
delegation = config.setdefault("delegation", {})
delegation.setdefault("inherit_mcp_toolsets", True)
delegation.setdefault("max_iterations", 50)
delegation.setdefault("child_timeout_seconds", 600)
delegation.setdefault("max_concurrent_children", 3)
delegation.setdefault("max_spawn_depth", 1)
delegation.setdefault("orchestrator_enabled", True)
delegation.setdefault("subagent_auto_approve", False)
...
if os.environ.get("TELEGRAM_BOT_TOKEN"):
    ...
    # Telegram allowed users (restrict bot access)
    ...
    if allowed_users:
        ...
        print(f"Telegram allowed_users: {allowed_users}")
...
send_startup_notification() {
  # Sends a startup notification to configured messaging platforms.
  # Called in background after gateway is ready (first boot only).
  local space_url="https://huggingface.co/spaces/${SPACE_ID:-unknown}"
  local model="${MODEL_FOR_CONFIG:-${HERMES_MODEL:-${LLM_MODEL:-unknown}}}"
  local provider="${PROVIDER_FOR_CONFIG:-auto}"
  local version="${HERMES_RUNTIME_VERSION:-unknown}"

  # Use Python for reliable JSON encoding and HTTP requests
  python3 - <<'NOTIFY_PY'
import os, json, urllib.request, sys

space_url = os.environ.get("SPACE_ID", "unknown")
space_url = f"https://huggingface.co/spaces/{space_url}" if space_url != "unknown" else "unknown"
model = os.environ.get("MODEL_FOR_CONFIG", os.environ.get("HERMES_MODEL", "unknown"))
provider = os.environ.get("PROVIDER_FOR_CONFIG", "auto")
import datetime
...
if tg_token:
    # TELEGRAM_BASE_URL may have /bot suffix (from cloudflare proxy setup), so use
    # CLOUDFLARE_PROXY_URL directly if available, otherwise fall back to standard API
    proxy_url = os.environ.get("CLOUDFLARE_PROXY_URL", "").strip().rstrip("/")
    if proxy_url:
        tg_base = proxy_url  # Proxy worker handles /bot{TOKEN}/... paths directly
    else:
        tg_base = "https://api.telegram.org"
    # Use TELEGRAM_ALLOWED_USERS as chat IDs (getUpdates conflicts with webhooks)
    allowed = os.environ.get("TELEGRAM_ALLOWED_USERS", "").strip()
    chat_ids = [x.strip() for x in allowed.split(",") if x.strip()] if allowed else []

    if not chat_ids:
        print("[startup-notify] No TELEGRAM_ALLOWED_USERS set, skipping Telegram notification")
    else:
        for cid in chat_ids:
            try:
                url = f"{tg_base}/bot{tg_token}/sendMessage"
                payload = json.dumps({"chat_id": cid, "text": msg}).encode()
                req = urllib.request.Request(url, data=payload,
                    headers={"Content-Type": "application/json", "User-Agent": "HuggingMes"})
                urllib.request.urlopen(req, timeout=15)
                print(f"[startup-notify] Sent Telegram notification to {cid}")
            except Exception as e:
                print(f"[startup-notify] Telegram send to {cid} failed: {e}")
...
if line_token:
    try:
        ...
        payload = json.dumps({"messages": [{"type": "text", "text": msg}]}).encode()
        req = urllib.request.Request(url, data=payload, headers={
            "Content-Type": "application/json",
            "Authorization": f"Bearer {line_token}",
        })
        urllib.request.urlopen(req, timeout=10)
        print("[startup-notify] Sent LINE broadcast notification")
    except Exception as e:
        print(f"[startup-notify] LINE send failed: {e}")
...
if discord_webhook:
    try:
        ...
        urllib.request.urlopen(req, timeout=10)
        print("[startup-notify] Sent Discord webhook notification")
    except Exception as e:
        print(f"[startup-notify] Discord webhook failed: {e}")
NOTIFY_PY
}
...
cleanup_gateway_processes() {
  # A pipeline or SIGTERM can leave the Hermes gateway child alive briefly; that
  # keeps Telegram/Discord bot tokens locked and breaks the next restart.
...
while true; do
  ...
  # ── Send startup notification to configured messaging platforms ──
