...
const API_SERVER_KEY = (process.env.API_SERVER_KEY || process.env.GATEWAY_TOKEN || "").trim();
console.log(`[health-server] API_SERVER_KEY configured: ${API_SERVER_KEY ? "yes" : "no"} (length=${API_SERVER_KEY.length})`);
const APP_BASE = "/app";
...
function timingSafeEqualString(left, right) {
  ...
  if (leftBuffer.length !== rightBuffer.length) {
    // Lengths don't match — guaranteed not equal, but still consume time
    // to avoid leaking length information.
    const maxLen = Math.max(leftBuffer.length, rightBuffer.length);
    const padLeft = Buffer.alloc(maxLen);
    const padRight = Buffer.alloc(maxLen);
    leftBuffer.copy(padLeft);
    rightBuffer.copy(padRight);
    crypto.timingSafeEqual(padLeft, padRight);
    return false;
  }
...
function buildSessionCookie(req) {
  // SameSite=None + Secure is required for HF Spaces because the app is
  // loaded inside a cross-origin iframe on huggingface.co, but the cookie
  // belongs to *.hf.space.  SameSite=Lax blocks cookie writes in this setup.
...
async function handleLogin(req, res, parsed) {
  ...
  try {
    ...
    const submittedToken = (params.get("token") || "").trim();
    const submittedNext = sanitizeNext(params.get("next") || nextPath);

    console.log(`[health-server] Login attempt: submitted_len=${submittedToken.length} expected_len=${API_SERVER_KEY.length}`);
    if (!timingSafeEqualString(submittedToken, API_SERVER_KEY)) {
      console.warn(`[health-server] Token check failed. Submitted token length: ${submittedToken.length}, Expected token length: ${API_SERVER_KEY.length}`);
      res.writeHead(401, {
        "content-type": "text/html; charset=utf-8",
        "cache-control": "no-store",
      });
...
function gatewayAuthHeader() {
  return API_SERVER_KEY ? { authorization: `Bearer ${API_SERVER_KEY}` } : {};
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (
    ...
    if (!requireAuth(req, res)) return;
    proxyRequest(req, res, DASHBOARD_PORT, (p) => p, gatewayAuthHeader());
    return;
  }
...
server.on("upgrade", (req, socket, head) => {
  ...
  const isGatewayApi = pathname === "/v1" || pathname.startsWith("/v1/");
  ...
  const targetPort = isJupyter
    ? JUPYTER_PORT
    : isApp || isDashboardAsset
      ? DASHBOARD_PORT
      : GATEWAY_PORT;
  const targetPath = isApp
    ? `${pathname.replace(/^\/app/, "") || "/"}${parsed.search}`
    : req.url;
  const rawJToken = (process.env.JUPYTER_TOKEN || "").trim();
  const upstreamAuth = isJupyter
    ? (rawJToken || API_SERVER_KEY ? `token ${rawJToken || API_SERVER_KEY}` : "")
    : API_SERVER_KEY
  ...
  const ps = net.createConnection(targetPort, GATEWAY_HOST, () => {
    ...
    if (upstreamAuth) {
      ps.write(`Authorization: ${upstreamAuth}\r\n`);
    }
    for (let i = 0; i < req.rawHeaders.length; i += 2) {
      const lower = req.rawHeaders[i].toLowerCase();
      if (["authorization", "host", "origin", "x-forwarded-host", "x-forwarded-proto"].includes(lower)) continue;
